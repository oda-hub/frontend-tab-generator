...
class MMODATabGenerator:
    ...
    def _arrange_data(self, instrument_name):
        ...
        for _item in jmeta[0]:
            ...
            if isinstance(item, list):
                if item[0]['query_name'] in ('src_query', 'instr_query'):
                    continue
                else:
    ...
    def generate(self, instrument_name, instruments_dir_path):
        param_dict, products_list = self._arrange_data(instrument_name)
        
        this_instr_path = os.path.join(instruments_dir_path, f"mmoda_{instrument_name}")
        os.makedirs(this_instr_path, exist_ok=True)
        basename = os.path.join(this_instr_path, f"mmoda_{instrument_name}")
        ...
        with open(f"{basename}.info", 'w') as fd:
            fd.write(templ.render(instrument_name = instrument_name))
...
def main():
    ...
    generator.generate(instrument_name, instruments_dir_path)
